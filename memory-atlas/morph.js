...
(() => {
  ...
  function draw(value){
    ...
    const {w,h}=bounds,mobile=w<650,p=ease(progress);
  ...
  function tick(now){if(!active||document.hidden||!motion)return;if(motion.last)motion.elapsed+=Math.min(now-motion.last,80);motion.last=now;
    const t=clamp((motion.elapsed-motion.hold)/motion.duration);draw(mix(motion.from,motion.to,ease(t)));
    if(t<1)raf=requestAnimationFrame(tick);else if(motion.to===0){motion={from:0,to:1,elapsed:0,last:0,hold:1300,duration:6200};raf=requestAnimationFrame(tick);}else{raf=0;motion=null;}
  }
  function play(){stop();if(!ready)return;if(reduced.matches){draw(1);return;}
    motion=progress>.01?{from:progress,to:0,elapsed:0,last:0,hold:0,duration:1600}:{from:0,to:1,elapsed:0,last:0,hold:2800,duration:6200};
  ...
  async function show(info){hide();const token=version;active=true;$('morph-controls').hidden=false;slider.disabled=true;replay.disabled=true;label.textContent='正在準備這一頁回憶…';
    try{const [original,illustration]=await preload(info);if(token!==version||!active)return;isSticker=info.id===2;
      ...
      draw(0);if(!reduced.matches)play();
